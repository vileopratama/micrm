...
class ResPartner(models.Model):
    ...
    bso_service_ids = fields.Many2many('crm.service', 'res_partner_service_rel',
                                                   'partner_id', 'service_id', string='Business Services & Outsourcing',
                                                   domain="[('type', '=', 'bso' )]", )
    ras_service_ids = fields.Many2many('crm.service', 'res_partner_service_rel',
                                       'partner_id', 'service_id', string='Risk Advisory Services',
                                       domain="[('type', '=', 'ras' )]", )
    it_advisory_service_ids = fields.Many2many('crm.service', 'res_partner_service_rel',
                                       'partner_id', 'service_id', string='IT Advisory',
                                       domain="[('type', '=', 'it-advisory' )]", )
    tax_service_ids = fields.Many2many('crm.service', 'res_partner_service_rel',
                                       'partner_id', 'service_id', string='Tax',
                                       domain="[('type', '=', 'tax' )]", )
    kjpp_service_ids = fields.Many2many('crm.service', 'res_partner_service_rel',
